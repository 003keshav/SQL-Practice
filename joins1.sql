-- 10/Feb/2026
-- Normalization :- A technique to store data in normal form where one data is not depend on other one and should not have dublicasy
-- 1NF:- single column should have single data
...
select * from orders;
-- orderid is primary key and candidate key in this table
-- customerid of customers table is foreign key which is connecting to customers
-- foreign key refers to primary key of another table

select o.orderid, o.pname, o.customerid from orders as o;
select c.name, c.city, c.customerid from customers as c;
 
-- join based on common column
select o.orderid, o.pname, o.customerid, c.name, c.city, c.customerid from orders as o
join customers as c
