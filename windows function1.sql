...
-- group by
select *, sum(amount) over() , sum(amount) over(partition by customer_name) from sales;

-- cumlative sum/ running sum 
-- a=10    20
-- b=10    20
-- c=20    20+20=40
select *, month(sale_date), sum(amount) over(order by amount) from sales;

select *, month(sale_date), sum(amount) over(partition by customer_name order by month(sale_date)) from sales;

select *, month(sale_date), sum(amount) over(partition by customer_name order by sale_date), 
sum(amount) over(partition by customer_name order by sale_date) - amount from sales;
...
select *,
rank() over(partition by customer_name order by amount) from sales;
