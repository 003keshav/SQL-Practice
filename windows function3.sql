...
select * from
(select *, dense_rank() over(partition by category order by amount desc) as ranks from  orders) as temp
where ranks<=2;

select order_id, customer_id, amount, total from
(select *, sum(amount) over(partition by customer_id order by order_id) as total from orders) as temp;

select order_id, amount, previous from
(select *, lag(amount,1) over(partition by customer_id order by order_id) as previous from orders) as temp;

select * from
(select *, avg(amount) over(partition by category) as avrage from orders) as temp;

select sum(percent) from
(select *, concat((amount/sum(amount) over())*100) as percent from orders) as temp;

select * from
(select *, dense_rank() over(partition by customer_id order by order_date) as first_order from orders) as temp
where first_order=1;

select distinct * from
 (select *, dense_rank() over(partition by city order by sum_city desc) as rnk from 
    (select *, sum(amount) over(partition by city, customer_id) as sum_city from orders) as temp)
as temp2 where rnk=1;

select *, dense_rank() over(partition by city order by total desc) from
 (select city, customer_id, sum(amount) as total
